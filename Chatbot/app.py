...
CORS(app, resources={r"/*": {"origins": "*"}})  # Allow CORS for all routess
...
def negotiate_price(user_input, current_product):
    ...
    if match:
        ...
        max_discounted_price = original_price * disc
...
def chat():
    ...
    return jsonify({'response': response, 'negotiation_state': negotiation_state})
